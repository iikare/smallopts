#!/bin/bash
# prefill.sh — Generic H200 prefill script (TP2 or TP4, single or multiple instances)
#
# Usage: bash prefill.sh [MODEL_NAME] [QUANTIZATION] [TP] [CUDA_DEVS] [PREFILL_ID] [CPUSET]
#
...
#   CUDA_DEVS:    GPU assignment                (default: 0,1,2,3 for TP4; use 0,1 for TP2)
#   PREFILL_ID:   Instance ID for multi-prefill (default: 1) — sets container name & side channel port
#   CPUSET:       CPU pinning                   (default: 0-55,112-167 for TP4 / 0-27,112-139 for TP2)
#
# Examples:
#   # 1P1D TP4
#   bash prefill.sh "meta-llama/Llama-3.1-70B-Instruct" fp8 4 "0,1,2,3" 1 "0-55,112-167"
#
#   # 1P3D TP2 (single prefill)
#   bash prefill.sh "RedHatAI/Qwen3-Next-80B-A3B-Instruct-FP8" none 2 "0,1" 1 "0-27,112-139"
#
#   # 3P1D TP2 (3 prefills)
#   bash prefill.sh "ModelName" none 2 "0,1" 1 "0-27,112-139"
#   bash prefill.sh "ModelName" none 2 "2,3" 2 "28-55,140-167"
#   bash prefill.sh "ModelName" none 2 "4,5" 3 "56-83,168-195"
...
CUDA_DEVS="${4:-0,1,2,3}"
PREFILL_ID="${5:-1}"
CPUSET="${6:-0-55,112-167}"

QUANT_FLAG=$([ "${QUANTIZATION}" = "none" ] && echo "" || echo "--quantization ${QUANTIZATION}")
CONTAINER_NAME="vllm-nvidia-gpu-dynamo-prefill${PREFILL_ID}-tp${TP}"
SIDE_CHANNEL_PORT=$((20090 + PREFILL_ID))
...
bash -lc 'python3 -m dynamo.vllm \
    ...
    --kv-transfer-config '"'"'{"kv_connector":"NixlConnector","kv_role":"kv_producer","kv_buffer_device":"cuda","enable_permute_local_kv":true,"kv_connector_extra_config":{"enforce_handshake_compat":false,"agreed_block_size":512}}'"'"''
