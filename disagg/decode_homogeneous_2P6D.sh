...
sudo docker stop vllm-nvidia-gpu-dynamo-decode1-tp2 vllm-nvidia-gpu-dynamo-decode2-tp2 vllm-nvidia-gpu-dynamo-decode3-tp2 2>/dev/null
sudo docker rm vllm-nvidia-gpu-dynamo-decode1-tp2 vllm-nvidia-gpu-dynamo-decode2-tp2 vllm-nvidia-gpu-dynamo-decode3-tp2 2>/dev/null

sudo docker run -itd \
--gpus all \
--cpuset-cpus=28-55,140-167 \
--network=host \
--ipc=host \
--ulimit memlock=-1:-1 \
--cap-add=SYS_ADMIN \
--group-add video \
--workdir /root \
--name vllm-nvidia-gpu-dynamo-decode1-tp2 \
...
--group-add video \
--workdir /root \
--name vllm-nvidia-gpu-dynamo-decode2-tp2 \
-e NATS_SERVER=nats://127.0.0.1:4222 \
-e ETCD_ENDPOINTS=http://127.0.0.1:2379 \
-e DYN_DISCOVERY_BACKEND=etcd \
-e DYN_REQUEST_PLANE=tcp \
...
-e VLLM_NIXL_BACKEND=UCX \
-e VLLM_NIXL_SIDE_CHANNEL_HOST=127.0.0.1 \
-e VLLM_NIXL_SIDE_CHANNEL_PORT=20098 \
-e VLLM_NIXL_DEVICE_TO_DEVICE=1 \
-e VLLM_SSM_CONV_STATE_LAYOUT=DS \
...
--group-add video \
--workdir /root \
--name vllm-nvidia-gpu-dynamo-decode3-tp2 \
-e NATS_SERVER=nats://127.0.0.1:4222 \
-e ETCD_ENDPOINTS=http://127.0.0.1:2379 \
-e DYN_DISCOVERY_BACKEND=etcd \
-e DYN_REQUEST_PLANE=tcp \
...
-e VLLM_NIXL_BACKEND=UCX \
-e VLLM_NIXL_SIDE_CHANNEL_HOST=127.0.0.1 \
-e VLLM_NIXL_SIDE_CHANNEL_PORT=20099 \
-e VLLM_NIXL_DEVICE_TO_DEVICE=1 \
-e VLLM_SSM_CONV_STATE_LAYOUT=DS \
